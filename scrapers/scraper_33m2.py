...
def search_list(
    ...
    headers = {
        ...
        "Origin": "https://33m2.co.kr",
        "Pragma": "no-cache",
        "Referer": f"https://33m2.co.kr/webpc/search/keyword?keyword={keyword}&start_date={start_date}&end_date={end_date}&week=1",
        "Sec-Ch-Ua": '"Not A(Brand";v="99", "Google Chrome";v="121", "Chromium";v="121"',
        "Sec-Ch-Ua-Mobile": "?0",
        "Sec-Ch-Ua-Platform": '"macOS"',
        "Sec-Fetch-Dest": "empty",
...
async def parse_detail(rid: int, with_img=True) -> dict:
    ...
    result["badges"] = [
        badge.text.strip() for badge in room_info0.select(".group .badge")
    ]
    ...
    for room_info in room_infos[1:]:
        ...
        if section_title == "공간 기본 정보":
            ...
            for li_tag in li_tags:
                res[li_tag.find("span").text.strip()] = li_tag.find(
                    "strong"
                ).text.strip()
            result["default_info"] = res
            result["default_keywords"] = [
                keyword.text.strip()
                for keyword in room_info.select(".keyword_area > span")
            ]
        elif section_title == "기본 옵션":
            result["default_options"] = [
                option.text.strip() for option in room_info.select("ul.option > li > p")
            ]
            ...
            result["location_text"] = room_info.find_all(class_="title_text")[
                -1
            ].text.strip()
            result["location_keywords"] = [
                keyword.text.strip()
                for keyword in room_info.select(".keyword_area > span")
            ]
            ...
            for li_tag in li_tags:
                review = {}
                review["score"] = (
                    li_tag.find(class_="score")["style"].split(": ")[1].split("%")[0]
                )
        ...
        for div in image_divs:
            ...
            if res.status_code == 200:
                ...
                async with aiofiles.open(
                    os.path.join(output_path, f"image_{image_count}.jpg"), "wb"
                ) as f:
