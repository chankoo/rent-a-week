...
class ViTRegressionPredictor(object):
    ...
    def train(
        self, target_key: str, test_size: float, random_state: int = 42, epochs: int = 5
    ):
        self.train_dataset, self.test_dataset = train_test_split(
            self.dataset, test_size=test_size, random_state=random_state
        )
        train_loader = self.dataloader_class(
        ...
        for epoch in range(epochs):
            ...
            for _, rent_infos, image_sets in train_loader:
                ...
                image_means = torch.stack(
                    [image_set.mean(dim=0) for image_set in image_sets], dim=0
                )

                # train
                self.optimizer.zero_grad()
                outputs = self.model(image_means).last_hidden_state.mean(
                    dim=1
                )  # 이미지 임베딩의 평균을 사용합니다.
                predictions = self.regression_layer(outputs)
                targets = torch.Tensor(
                    [rent_info[target_key] for rent_info in rent_infos]
                ).view(-1, 1)
    ...
    def eval(self, target_key: str):
        test_loader = self.dataloader_class(
        ...
        with torch.no_grad():
            for _, rent_infos, image_sets in test_loader:
                ...
                image_means = torch.stack(
                    [
                        torch.stack(image_set, dim=0).mean(dim=0)
